...
class Unpacker(ServiceBase):
    ...
    def execute(self, request):
        ...
        if uresult.ok and uresult.localpath:
            ...
            request.result.add_section(
                ResultSection(f"{os.path.basename(uresult.displayname)} successfully unpacked!", body=caveat_msg)
            )

    def _check_upx(self, packedfile):
        # Test the file to see if UPX agrees with our identification
        stdout, stderr = Popen((self.upx_exe, "-t", packedfile), stdout=PIPE, stderr=PIPE).communicate()
    ...
    def _unpack_upx(self, packedfile, outputpath, displayname):
        ...
        if check is True:
            stdout, stderr = Popen(
                (self.upx_exe, "-d", "-o", outputpath, packedfile), stdout=PIPE, stderr=PIPE
            ).communicate()
        ...
        # Check is True when UPX -t validates file - we could try to unpack
            ...
            self.log.info(
                f"UPX extractor said this file was not UPX packed:\n{check_stderr[:1024]}\n{check_stderr[:1024]}"
            )
