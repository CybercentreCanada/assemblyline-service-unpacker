...
UnpackResult = namedtuple('UnpackResult', ['ok', 'filepath', 'meta'])
...
class Unpacker(ServiceBase):
    ...
    SERVICE_REVISION = ServiceBase.parse_revision('$Id: 3da3b153a64ab046981d918f57d081d6c6eb3d04 $')
    ...
    def execute(self, request):
        ...
        if uresult.ok and uresult.filepath:
            request.add_extracted(uresult.filepath, 'Unpacked from %s' % request.srl)
            request.result.add_section(ResultSection(SCORE.INFO,
                                                     "%s successfully unpacked!" % (os.path.basename(uresult.filepath)),
                                       self.SERVICE_CLASSIFICATION))

    def _unpack_upx(self, packedfile, outputpath):
        ...
        if '[OK]' in stdout and 'Tested 1 file' in stdout:
            ...
            if 'Unpacked 1 file' in stdout:
                ...
                return UnpackResult(True, outputpath, {'stdout': stdout[:1024]})
        else:
        ...
        return UnpackResult(True, None, None)

    def _unpack(self, request, packer_names):
        packer = PACKER_UNKNOWN
        for name in packer_names:
            if 'upx' in name.lower():
                packer = PACKER_UPX
                break
        # for right now we only support upx.
        if packer != PACKER_UPX:
            return UnpackResult(True, None, None)

        packedfile = request.download()
        unpackedfile = packedfile + '.unpacked'
        return self._unpack_upx(packedfile, unpackedfile)
